...
class WhatsAppCloudAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    SEMANTIC_EXACT_ATTEMPT_CAPABILITY = LiveSemanticExactAttemptCapability(
        ...
        segmentation_version="whatsapp-cloud-logical-v1",
        max_logical_units=4000,
        length_semantics="unicode_codepoints",
        wire_encoding="whatsapp-cloud-text-json-v1",
    )
    ...
    async def send_semantic_exact_attempt(self, request) -> SendResult:
        """Send one unformatted Graph text payload and require a real wamid."""
        ...
        if (
            ...
            or not request.chat_id.isdigit()
            or not 7 <= len(request.chat_id) <= 15
        ...
        try:
            ...
            return SendResult(
                success=False,
                error="WhatsApp Cloud semantic delivery timed out",
            )
        except Exception as exc:
            return SendResult(
                success=False,
                error=f"WhatsApp Cloud semantic transport failed: {exc}",
            )
        ...
        if 200 <= status < 300:
            ...
            if isinstance(messages, list) and messages:
                ...
                if isinstance(first, dict):
                    message_id = exact_provider_message_id(first.get("id"))
        ...
        if status == 429:
            ...
            return SendResult(
                success=False,
                error="WhatsApp Cloud rate limited semantic delivery",
                retryable=True,
