...
_TWILIO_MESSAGE_SID_RE = re.compile(r"^(?:SM|MM)[0-9a-fA-F]{32}$")
...
class SmsAdapter(BasePlatformAdapter):
    ...
    SEMANTIC_EXACT_ATTEMPT_CAPABILITY = LiveSemanticExactAttemptCapability(
        ...
        segmentation_version="twilio-sms-logical-v1",
        max_logical_units=1_400,
        length_semantics="unicode_codepoints",
        wire_encoding="twilio-message-form-v1",
    )
    ...
    async def send_semantic_exact_attempt(self, request) -> SendResult:
        """Create exactly one Twilio Message resource without redirects."""
        ...
        account_sid_valid = bool(
            self._account_sid.startswith("AC")
            and self._account_sid.isalnum()
        )
        try:
        ...
        if (
            ...
            or not _is_e164(request.chat_id)
            or not _is_e164(self._from_number)
        ...
        form_data.add_field("To", request.chat_id)
        form_data.add_field("Body", request.content)
        ...
        try:
            ...
            async with session.post(
                ...
                if 200 <= status < 300:
                    ...
                    message_id = (
                        ...
                        and _TWILIO_MESSAGE_SID_RE.fullmatch(candidate)
                        else None
                    )
                ...
                if status == 429:
                    ...
                    return SendResult(
                        success=False,
                        error="Twilio rate limited semantic delivery",
                        retryable=True,
            ...
            return SendResult(
                success=False,
                error=f"Twilio semantic transport failed: {exc}",
            )
